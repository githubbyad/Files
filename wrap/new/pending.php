...
// pending orders
$pendingSql = $conn->prepare("SELECT * FROM orders WHERE order_pending_status = :p AND order_delete = ''");
...
<div class="row mx-0">
    ...
    <div class="col-lg-12 pending_div position-relative">
        ...
        <dir class="row px-0">
            <?php
            $orderSql = $conn->prepare("SELECT * FROM orders WHERE order_pending_status = :status AND order_delete = '' ORDER BY order_timestamp DESC");
            ...
            if (count($orderObj)) {
                foreach ($orderObj as $row) {
                    ...
                    //  delete button
                    $deleteButton = "";
                    $hide_after_minutes = $setting->first()->hide_delete_button_after;
                    $timeNow = explode(' ', $timeDiff)[0];                
                    if (str_contains($timeDiff, 'seconds ago') || (str_contains($timeDiff, 'minutes ago') && $timeNow <= $hide_after_minutes)) {
                        $deleteButton = "<button data-id='{$row->order_id}' data-order='{$row->order_number}' class='deleteItem col btn btn-danger ms-1'>Delete</button>";
                    }
                    ...
                    $tableData = "<div class='pendingDiv col-12 col-sm-6 col-lg-2' data-time='{$timeDiff}'>
                                    <div class='orderWrap w-100 p-2 mb-5 mb-sm-3 bg-body rounded shadow text-center'>
                                        ...
                                        <p class='view_delete_wrap text-center row mx-0 mb-1 d-none d-lg-flex'><button data-id='{$row->order_id}' class='viewItem col btn btn-dark'>View</button>" . $deleteButton . "</p>                                                                                
                                    </div>
                                </div>";
                    echo $tableData;
                }
                ...
                <table class="table orderTable table-hover table-borderless">
                    ...
                    <tbody>
                        ...
                        ?>
                            ...
                            <tr class="<?= $row->order_pending_status == 'Deleted' ? 'deleted_order' : '' ?>">
                                ...
                                <td class="text-center"><?= $row->order_pending_status == "" ? "<st class='st-success'>Done<st>" : ($row->order_pending_status == "Pending" ? "<st class='st-warning'>Pending</st>" : "<st class='st-delete'>Deleted</st>") ?></td>
